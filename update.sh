...
# 2. Get Latest Tag
LATEST_VERSION=$(curl -s "https://api.github.com/repos/EasyTier/EasyTier/releases/latest" | grep '"tag_name":' | sed -E 's/.*"tag_name": *"([^"]+)".*/\1/')
...
if [ -f "$BIN_PATH/easytier-core" ]; then
    ...
    if [ "$CURRENT_VERSION" == "$LATEST_VERSION" ]; then
        echo "Already at latest version ($CURRENT_VERSION). Skipping update."
        exit 0
    fi
fi
