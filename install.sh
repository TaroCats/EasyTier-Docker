...
GET_LATEST_TAG() {
  ...
  RESPONSE=$(curl -s "https://api.github.com/repos/EasyTier/EasyTier/tags?per_page=1")
  LATEST_VERSION=$(echo "$RESPONSE" | grep -m 1 '"name":' | sed -E 's/.*"name": *"([^"]+)".*/\1/')
...
INSTALL() {
  # Get version number (include pre-release)
